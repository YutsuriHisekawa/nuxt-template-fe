  ...
  <template v-else-if="block.type === 'table'">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
      <div class="space-y-1.5">
        ...
        <input type="number" min="1" max="50" :value="block.rows" @input="update('rows', Number($event.target.value))" class="h-9 w-full rounded-md border border-border bg-background px-3 text-sm" />
      </div>
      <div class="space-y-1.5">
        <label class="text-xs font-semibold text-muted-foreground">Kolom</label>
        <input type="number" min="1" max="10" :value="block.cols" @input="update('cols', Number($event.target.value))" class="h-9 w-full rounded-md border border-border bg-background px-3 text-sm" />
      </div>
    ...
    <div class="space-y-1.5">
      ...
      <textarea :value="block.cellsText" @input="update('cellsText', $event.target.value)" rows="6" class="w-full rounded-md border border-border bg-background px-3 py-2 text-xs font-mono" />
      <p class="text-[11px] text-muted-foreground">Satu baris = satu row. Kolom dipisahkan <code>|</code>. Contoh: <code>Nama|Qty|Harga</code></p>
    </div>
  </template>
