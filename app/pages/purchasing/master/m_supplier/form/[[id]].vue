...
const onSave = async () => {
  ...
  try {
    ...
    nama_kuto: values.nama_kuto?.toString().trim() || null,
        ...
        <CardContent class="space-y-6">
          ...
          <div v-show="wizardStep === 2" class="grid grid-cols-1 gap-6">
            ...
            <FieldPopUp
              id="nama_kuto"
              label="Nama Kota"
              :value="values.nama_kuto"
              :errorname="errors.nama_kuto ? 'failed' : ''"
              @input="(v) => (values.nama_kuto = v)"
              :hints="errors.nama_kuto"
              :required="false"
              :disabled="loading || isReadOnly"
              :readonly="isReadOnly"
              apiUrl="/api/dynamic/m_kuto"
              displayField="nama_kuto"
              valueField="id"
              :columns="[{ field: 'kode_kuto', headerName: 'Kode Kota' }, { field: 'nama_kuto', headerName: 'Nama Kota' }]"
