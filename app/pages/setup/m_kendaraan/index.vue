...
const landing = reactive({
	...
	columns: [
		{ headerName: "Kode Kendaraan", field: "kode_kendaraan", minWidth: 140 },
		{ headerName: "Jenis", valueGetter: (p) => p.data?.jenis_kendaraan?.value1, minWidth: 140 },
		{ headerName: "Nama Kendaraan", field: "nama_kendaraan", minWidth: 140 },
		{ headerName: "Company", valueGetter: (p) => p.data?.m_unit_bisni?.nama_comp, minWidth: 140 },
		{ headerName: "No Polisi", field: "no_polisi", minWidth: 140 },
		{ headerName: "No STNK (Opsional)", field: "no_stnk", minWidth: 140 },
		{ headerName: "No Mesin", field: "no_mesin", minWidth: 140 },
		{
			headerName: "Aktif",
			field: "is_active",
			minWidth: 110,
...
<template>
	<div class="space-y-4 pt-5">
		<!-- Page Header -->
		<div class="px-1">
			<h1 class="text-xl sm:text-2xl font-bold tracking-tight">Master Kendaraan</h1>
			<p class="text-xs sm:text-sm text-muted-foreground">Kelola master kendaraan aplikasi</p>
		</div>
		...
		<div class="rounded-lg border border-border bg-card p-2">
			...
			<div v-if="!isMobile">
				<DataTable
					...
					:showSearch="true"
					:showRowNumber="true"
					searchPlaceholder="Search master kendaraan..."
				...
				<div class="flex items-center gap-2">
					<form class="flex flex-1 items-center gap-2" @submit.prevent="onMobileSearch">
						<input
							v-model="searchInput"
							type="text"
							placeholder="Search master kendaraan..."
					...
					>
						...
						<div class="flex items-start justify-between gap-2">
							<div class="min-w-0 flex-1">
								<p class="text-sm font-semibold truncate">{{ row.kode_kendaraan || '-' }}</p>
								<p class="text-xs text-muted-foreground truncate">{{ row.jenis_kendaraan_id || '-' }}</p>
							</div>
							<span
						...
						<div class="grid grid-cols-1 gap-y-0.5 text-xs">
							<div v-if="row.nama_kendaraan"><span class="text-muted-foreground">Nama Kendaraan:</span> <span class="font-medium">{{ row.nama_kendaraan }}</span></div>
							<div v-if="row.m_unit_bisni.nama_comp"><span class="text-muted-foreground">Company:</span> <span class="font-medium">{{ row.m_unit_bisni.nama_comp }}</span></div>
							<div v-if="row.no_polisi"><span class="text-muted-foreground">No Polisi:</span> <span class="font-medium">{{ row.no_polisi }}</span></div>
						</div>
				...
				<AlertDialogContent class="max-w-[90vw] sm:max-w-lg">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Data "{{ deleteTarget?.kode_kendaraan || deleteTarget?.id || '-' }}" akan dihapus permanen.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
