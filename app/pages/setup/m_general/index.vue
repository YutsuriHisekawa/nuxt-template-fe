...
const landing = reactive({
	api: {
		url: "/api/dynamic/m_general",
		searchFields: ["group", "key1", "code", "value1", "value2"],
	...
	columns: [
		{ headerName: "Group", field: "group", minWidth: 140 },
		{ headerName: "Key", field: "key1", minWidth: 140 },
		{ headerName: "Code", field: "code", minWidth: 120 },
		{ headerName: "Value 1", field: "value1", minWidth: 160 },
		{ headerName: "Value 2", field: "value2", minWidth: 140 },
		{ headerName: "Value 3", field: "value3", minWidth: 120 },
		{
			headerName: "Aktif",
			field: "is_active",
			minWidth: 110,
...
<template>
	<div class="space-y-4 pt-5">
		<!-- Page Header -->
		<div class="px-1">
			<h1 class="text-xl sm:text-2xl font-bold tracking-tight">Master General</h1>
			<p class="text-xs sm:text-sm text-muted-foreground">Kelola General aplikasi</p>
		</div>
		...
		<div class="rounded-lg border border-border bg-card p-2">
			...
			<div v-if="!isMobile">
				<DataTable
					...
					:showSearch="true"
					:showRowNumber="true"
					searchPlaceholder="Search general..."
			...
			<div v-else class="space-y-3 px-1">
				...
				<div v-else class="space-y-2">
					...
					>
						<!-- Header: Group + Key + Status -->
						<div class="flex items-start justify-between gap-2">
							<div class="min-w-0 flex-1">
								<p class="text-sm font-semibold truncate">{{ row.group || '-' }}</p>
								<p class="text-xs text-muted-foreground truncate">{{ row.key1 || '-' }} {{ row.code ? '• ' + row.code : '' }}</p>
							</div>
							<span
						...
						<div class="grid grid-cols-1 gap-y-0.5 text-xs">
							<div v-if="row.value1"><span class="text-muted-foreground">Val 1:</span> <span class="font-medium">{{ row.value1 }}</span></div>
							<div v-if="row.value2"><span class="text-muted-foreground">Val 2:</span> <span class="font-medium">{{ row.value2 }}</span></div>
							<div v-if="row.value3"><span class="text-muted-foreground">Val 3:</span> <span class="font-medium">{{ row.value3 }}</span></div>
						</div>
				...
				<AlertDialogContent class="max-w-[90vw] sm:max-w-lg">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Data "{{ deleteTarget?.group || '' }} - {{ deleteTarget?.key1 || deleteTarget?.id || '-' }}" akan dihapus permanen.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
