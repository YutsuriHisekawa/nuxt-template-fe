...
const landing = reactive({
	...
	columns: [
		{ headerName: "Company", field: "m_unit_bisni.nama_comp", minWidth: 140 },
		{ headerName: "Kode Karyawan", field: "kode_kary", minWidth: 140 },
		{ headerName: "Nama Karyawan", field: "nama_kary", minWidth: 140 },
		{ headerName: "Email", field: "email", minWidth: 140 },
		{ headerName: "Nomor Telepon", field: "no_tlp", minWidth: 140 },
		{ headerName: "Jabatan", field: "posisi.value1", minWidth: 140 },
		{
			headerName: "Aktif",
			field: "is_sales",
			minWidth: 110,
...
<template>
	<div class="space-y-4 pt-5">
		<!-- Page Header -->
		<div class="px-1">
			<h1 class="text-xl sm:text-2xl font-bold tracking-tight">Master Kary</h1>
			<p class="text-xs sm:text-sm text-muted-foreground">Kelola master kary aplikasi</p>
		</div>
		...
		<div class="rounded-lg border border-border bg-card p-2">
			...
			<div v-if="!isMobile">
				<DataTable
					...
					:showSearch="true"
					:showRowNumber="true"
					searchPlaceholder="Search master kary..."
				...
				<div class="flex items-center gap-2">
					<form class="flex flex-1 items-center gap-2" @submit.prevent="onMobileSearch">
						<input
							v-model="searchInput"
							type="text"
							placeholder="Search master kary..."
					...
					>
						...
						<div class="flex items-start justify-between gap-2">
							<div class="min-w-0 flex-1">
								<p class="text-sm font-semibold truncate">{{ row.kode_kary || '-' }}</p>
								<p class="text-xs text-muted-foreground truncate">{{ row.nama_kary || '-' }}</p>
							</div>
							<span
						...
						<div class="grid grid-cols-1 gap-y-0.5 text-xs">
							<div v-if="row.no_ktp"><span class="text-muted-foreground">Nomor KTP:</span> <span class="font-medium">{{ row.no_ktp }}</span></div>
							<div v-if="row.foto_ktp"><span class="text-muted-foreground">Foto KTP:</span> <span class="font-medium">{{ row.foto_ktp }}</span></div>
							<div v-if="row.tempat_lhr"><span class="text-muted-foreground">Tempat Lahir:</span> <span class="font-medium">{{ row.tempat_lhr }}</span></div>
						</div>
				...
				<AlertDialogContent class="max-w-[90vw] sm:max-w-lg">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Data "{{ deleteTarget?.kode_kary || deleteTarget?.id || '-' }}" akan dihapus permanen.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
