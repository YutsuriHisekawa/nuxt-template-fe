...
const landing = reactive({
	...
	columns: [
		{ headerName: "Kode", field: "kode_approval", minWidth: 140 },
		{ headerName: "Nama Approval", field: "nama_approval", minWidth: 140 },
		{ headerName: "Menu", valueGetter: (p) => p.data?.m_menu?.name, minWidth: 140 },
...
<template>
	<div class="space-y-4 pt-5">
		<!-- Page Header -->
		<div class="px-1">
			<h1 class="text-xl sm:text-2xl font-bold tracking-tight">Master Approval</h1>
			<p class="text-xs sm:text-sm text-muted-foreground">Kelola master approval aplikasi</p>
		</div>
		...
		<div class="rounded-lg border border-border bg-card p-2">
			...
			<div v-if="!isMobile">
				<DataTable
					...
					:showSearch="true"
					:showRowNumber="true"
					searchPlaceholder="Search master approval..."
				...
				<div class="flex items-center gap-2">
					<form class="flex flex-1 items-center gap-2" @submit.prevent="onMobileSearch">
						<input
							v-model="searchInput"
							type="text"
							placeholder="Search master approval..."
					...
					>
						...
						<div class="flex items-start justify-between gap-2">
							<div class="min-w-0 flex-1">
								<p class="text-sm font-semibold truncate">{{ row.kode_approval || '-' }}</p>
								<p class="text-xs text-muted-foreground truncate">{{ row.nama_approval || '-' }}</p>
							</div>
							<span
						...
						<div class="grid grid-cols-1 gap-y-0.5 text-xs">
							<div v-if="row.m_menu.name"><span class="text-muted-foreground">Menu:</span> <span class="font-medium">{{ row.m_menu.name }}</span></div>
				...
				<AlertDialogContent class="max-w-[90vw] sm:max-w-lg">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Data "{{ deleteTarget?.kode_approval || deleteTarget?.id || '-' }}" akan dihapus permanen.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
