...
const landing = reactive({
	...
	columns: [
		{ headerName: "Kode Unit Bisnis", field: "kode_unit_bisnis", minWidth: 140 },
		{ headerName: "Nama Unit Bisnis", field: "nama_comp", minWidth: 140 },
		{ headerName: "Provinsi", field: "provinsi", minWidth: 140 },
		{ headerName: "Kota", field: "kota", minWidth: 140 },
		{ headerName: "Kecamatan", field: "kecamatan", minWidth: 140 },
		{ headerName: "Kode-Pos", field: "kodepos", minWidth: 140 },
		{ headerName: "Email", field: "email", minWidth: 140 },
		{ headerName: "PIC", field: "pic", minWidth: 140 },
		{ headerName: "No.PIC", field: "no_telp_pic", minWidth: 140 },
...
<template>
	<div class="space-y-4 pt-5">
		<!-- Page Header -->
		<div class="px-1">
			<h1 class="text-xl sm:text-2xl font-bold tracking-tight">Master Unit Bisnis</h1>
			<p class="text-xs sm:text-sm text-muted-foreground">Kelola master unit bisnis aplikasi</p>
		</div>
		...
		<div class="rounded-lg border border-border bg-card p-2">
			...
			<div v-if="!isMobile">
				<DataTable
					...
					:showSearch="true"
					:showRowNumber="true"
					searchPlaceholder="Search master unit bisnis..."
				...
				<div class="flex items-center gap-2">
					<form class="flex flex-1 items-center gap-2" @submit.prevent="onMobileSearch">
						<input
							v-model="searchInput"
							type="text"
							placeholder="Search master unit bisnis..."
					...
					>
						...
						<div class="flex items-start justify-between gap-2">
							<div class="min-w-0 flex-1">
								<p class="text-sm font-semibold truncate">{{ row.kode_unit_bisnis || '-' }}</p>
								<p class="text-xs text-muted-foreground truncate">{{ row.nama_comp || '-' }}</p>
							</div>
							<span
						...
						<div class="grid grid-cols-1 gap-y-0.5 text-xs">
							<div v-if="row.provinsi"><span class="text-muted-foreground">Provinsi:</span> <span class="font-medium">{{ row.provinsi }}</span></div>
							<div v-if="row.kota"><span class="text-muted-foreground">Kota:</span> <span class="font-medium">{{ row.kota }}</span></div>
							<div v-if="row.kecamatan"><span class="text-muted-foreground">Kecamatan:</span> <span class="font-medium">{{ row.kecamatan }}</span></div>
						</div>
				...
				<AlertDialogContent class="max-w-[90vw] sm:max-w-lg">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Data "{{ deleteTarget?.kode_unit_bisnis || deleteTarget?.id || '-' }}" akan dihapus permanen.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
