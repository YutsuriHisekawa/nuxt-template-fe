...
const landing = reactive({
	...
	columns: [
		{ headerName: "Catatan", field: "note", minWidth: 140 },
		{ headerName: "Total SO", field: "amount", minWidth: 140 },
	],
})
...
<template>
	<div class="space-y-4 pt-5">
		<!-- Page Header -->
		<div class="px-1">
			<h1 class="text-xl sm:text-2xl font-bold tracking-tight">Transaksi Sales Order</h1>
			<p class="text-xs sm:text-sm text-muted-foreground">Kelola transaksi sales order aplikasi</p>
		</div>
		...
		<div class="rounded-lg border border-border bg-card p-2">
			...
			<div v-if="!isMobile">
				<DataTable
					...
					:showSearch="true"
					:showRowNumber="true"
					searchPlaceholder="Search transaksi sales order..."
				...
				<div class="flex items-center gap-2">
					<form class="flex flex-1 items-center gap-2" @submit.prevent="onMobileSearch">
						<input
							v-model="searchInput"
							type="text"
							placeholder="Search transaksi sales order..."
					...
					>
						...
						<div class="flex items-start justify-between gap-2">
							<div class="min-w-0 flex-1">
								<p class="text-sm font-semibold truncate">{{ row.note || '-' }}</p>
								<p class="text-xs text-muted-foreground truncate">{{ row.amount || '-' }}</p>
							</div>
							<span
				...
				<AlertDialogContent class="max-w-[90vw] sm:max-w-lg">
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Data "{{ deleteTarget?.note || deleteTarget?.id || '-' }}" akan dihapus permanen.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
