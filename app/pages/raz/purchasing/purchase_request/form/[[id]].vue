...
const currentMenu = computed(() => {
  return authStore.selectRespo?.menus?.find(m => m.path === '/raz/purchasing/purchase_request');
});
...
const pageTitle = computed(() => {
  if (isViewMode.value) return "Detail Master Purchase Request";
  if (isCopyMode.value) return "Salin Master Purchase Request";
  if (isEditMode.value) return "Edit Master Purchase Request";
  return "Tambah Master Purchase Request Baru";
});

const pageDescription = computed(() => {
  if (isViewMode.value) return "Lihat detail data master purchase request";
  if (isCopyMode.value) return "Buat data baru dari data yang disalin";
  if (isEditMode.value) return "Perbarui data master purchase request";
  return "Buat data master purchase request baru";
});
...
const selectedDetailIds = computed(() => detailArr.value.map(d => d.m_items_id));
...
const handleDetailAdd = (selectedItems) => {
  ...
  selectedItems.forEach((item) => {
    const exists = detailArr.value.some((d) => d.m_items_id === item.id);
...
// ============================================================================
const API_BASE = "/api/dynamic/purchase_request";
...
onBeforeMount(async () => {
  ...
  try {
    ...
    setTimeout(() => router.push("/raz/purchasing/purchase_request"), 2000);
  }
  loading.value = false;
});
  ...
  try {
    ...
    kota_supp_id: values.kota_supp_id?.toString().trim() || null,
      ...
      detail: detailArr.value.map((d) => ({
        m_items_id: d.m_items_id,
        nama_item: d.nama_item?.toString().trim() || null,
    ...
    router.replace("/raz/purchasing/purchase_request");
  } catch (error) {
...
// ============================================================================
const handleCancel = () => {
  router.push("/raz/purchasing/purchase_request");
};
</script>
  ...
  <div class="p-6 space-y-6">
    ...
    <form class="space-y-6" @submit.prevent>
      <Card :key="recordId || 'new'">
        <CardHeader>
          <CardTitle>Informasi Master Purchase Request</CardTitle>
          <CardDescription>
            Isi data master purchase request dengan lengkap dan benar
          </CardDescription>
        </CardHeader>
            ...
            <CardHeader>
              <div class="flex items-center justify-between">
                ...
                <ButtonMultiSelect
                  v-if="!isReadOnly"
                  title="Pilih Item"
                  :api="{ url: '/api/dynamic/m_items?include=stock_gudang,m_general&no_pagination=true' }"
                  :columns="[
                    { key: 'kode_item', label: 'Kode Item', sortable: true, width: '200px' },
                    { key: 'nama_item', label: 'Nama Item', sortable: true, width: '200px' },
                    { key: 'uom.value1', label: 'Unit', sortable: true, width: '200px' },
                    { key: 'stock_gudang.free_stock', label: 'Qty Stock', sortable: true, width: '200px' },
                    { key: 'stock_gudang.m_gudang_id', label: 'Gudang', sortable: true, width: '200px' }
                  ]"
                  searchKey="nama_item"
                  displayKey="Nama Item"
                  uniqueKey="id"
                ...
                <table class="w-full text-sm">
                  ...
                  <tbody>
                    <tr v-if="detailArr.length === 0">
                      <td colspan="9" class="text-center py-8 text-muted-foreground text-xs sm:text-sm">
                    ...
                    <tr v-for="(detail, index) in detailArr" :key="index" class="border-t hover:bg-muted/50">
                      ...
                      <td class="px-2 py-2" style="width: 180px; min-width: 180px;">
                        <FieldX
                          ...
                          @input="(v) => (detail.nama_item = v)"
                          placeholder="Nama Item Print"
                          :readonly="false"
                          class="w-full"
                        />
                        <span v-else>{{ detail.nama_item || '-' }}</span>
                      </td>
