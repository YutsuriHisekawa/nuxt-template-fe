...
const landing = reactive({
	api: {
		url: "/api/dynamic/purchase_request",
	...
	columns: [
		...
		{ headerName: "Kode Coba", valueGetter: (p) => p.data?.m_coba_rel?.kode_coba, minWidth: 140 },
		{ headerName: "Nama Coba", valueGetter: (p) => p.data?.m_coba_rel?.nama_coba, minWidth: 140 },
		{ headerName: "Negara Vendor", valueGetter: (p) => p.data?.m_negoro?.nama_negoro, minWidth: 140 },
		{ headerName: "Kota Vendor", valueGetter: (p) => p.data?.m_kuto?.nama_kuto, minWidth: 140 },
	],
})
...
<template>
	<div class="space-y-4 pt-5">
		<!-- Page Header -->
		<div class="px-1">
			<h1 class="text-xl sm:text-2xl font-bold tracking-tight">Master Purchase Request</h1>
			<p class="text-xs sm:text-sm text-muted-foreground">Kelola master purchase request aplikasi</p>
		</div>
		...
		<div class="rounded-lg border border-border bg-card p-2">
			...
			<div v-if="!isMobile">
				<DataTable
					...
					:showSearch="true"
					:showRowNumber="true"
					searchPlaceholder="Search master purchase request..."
				...
				<div class="flex items-center gap-2">
					<form class="flex flex-1 items-center gap-2" @submit.prevent="onMobileSearch">
						<input
							v-model="searchInput"
							type="text"
							placeholder="Search master purchase request..."
				...
				<div v-else class="space-y-2">
					...
					>
						...
						<div class="grid grid-cols-1 gap-y-0.5 text-xs">
							...
							<div v-if="row.m_coba_rel.kode_coba"><span class="text-muted-foreground">Kode Coba:</span> <span class="font-medium">{{ row.m_coba_rel.kode_coba }}</span></div>
							<div v-if="row.m_coba_rel.nama_coba"><span class="text-muted-foreground">Nama Coba:</span> <span class="font-medium">{{ row.m_coba_rel.nama_coba }}</span></div>
						</div>
